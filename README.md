...
[在线阅读完整教程](https://petemeng.github.io/microbiome-best-practices/)

技术章节提供本例数据入口，从公开表格读入开始，按“数据 → 分析 → 图表 → 解释”推进。
导读和证据阅读章节直接展示案例，不要求安装统计环境。先按科学问题选择章节，
再根据设计、输入单位与推断目标判断方法是否适用。
...
- 通过检查的内容按项目授权同步 GitHub 与对应微信草稿；新稿核对通过后才替换映射旧稿。发布与群发仍需另行授权。
- 线上网站部署已验证的静态产物到 `gh-pages`；传统源码 CI 的 deploy job 仍默认关闭，不能将其构建状态与静态网站部署混为一谈。
...
- 各章进一步说明了方法的统计对象、分母、独立单位、关键假设与结论边界。例如排序区分距离失真与分组检验，组成图区分等样本均值与 pooled reads，机器学习区分内部验证与跨队列迁移。
- 网页数学表达使用 Quarto 的标准公式语法；微信端不依赖 MathJax，公式转为图片，正文图和代表性结果封面保持分离。
- 网站的 `version.json` 记录部署对应的源代码版本；数据和读者分析脚本仍以各章固定的公开链接为准。
