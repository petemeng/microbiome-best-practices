...
|---|---|---|
| `otutab` | 行是 ASV/OTU，列是样本，单元格是 reads 数 | 保留非负整数计数，不要用百分比代替原始计数 |
| `taxonomy` | 行是同一批 ASV/OTU，列是分类层级 | 特征 ID 与计数表一致；未注释的分类保留为未知 |
| `metadata` | 行是样本，列是分组、个体、时间、批次及协变量 | 样本 ID 与计数表一致；技术重复不能当作独立样本 |

可以先查看中国湿地数据的[计数表](https://raw.githubusercontent.com/petemeng/microbiome-best-practices/3cb6a817e0c73ab7ecbec1cedc1ad80cbb9ecfaa/data/small/otutab.tsv)、[分类表](https://raw.githubusercontent.com/petemeng/microbiome-best-practices/3cb6a817e0c73ab7ecbec1cedc1ad80cbb9ecfaa/data/small/taxonomy.tsv)和[样本信息表](https://raw.githubusercontent.com/petemeng/microbiome-best-practices/3cb6a817e0c73ab7ecbec1cedc1ad80cbb9ecfaa/data/small/metadata.tsv)：计数表包含 13,628 个 OTU、90 个样本，分为 inland、coastal 和 Tibetan wetland 三组，每组 30 个样本。
