#!/usr/bin/env python3
"""Combine the established 01–29 QA with release gates for Articles 30–45."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output", type=Path,
        default=Path("results/articles-01-45-release-validation.json"),
    )
...
def main() -> int:
    ...
    chapters = manifest["series"]["chapters"][:45]
    components = {
    ...
    for name, path in components.items():
        ...
        component_records[name] = {
            "path": str(path), "sha256": sha256(path), "status": payload.get("status"),
            "checks_passed": payload.get("checks_passed"),
            "checks_failed": payload.get("checks_failed"),
        }
        check(f"component-{name}-passed", payload.get("status") == "passed", payload.get("status"))

    scaffold = json.loads(components["series_contract"].read_text(encoding="utf-8"))
    check("formal-series-coverage", scaffold.get("pilot_articles") == list(range(1, 46)), scaffold.get("pilot_articles"))
    check("manifest-formal-count", len(chapters) == 45, len(chapters))
    check("manifest-total-count", manifest["series"]["total_articles"] == 55, manifest["series"]["total_articles"])

    html_count = 0
    for chapter in chapters:
        source = Path(chapter["file"])
        html = root / "_site" / ("index.html" if source.name == "index.qmd" else source.with_suffix(".html"))
    ...
    check("formal-html-count", html_count == 45, html_count)
    ...
    payload = {
        ...
        "scope": "Articles 01–45 local release assets",
