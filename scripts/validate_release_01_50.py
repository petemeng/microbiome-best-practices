#!/usr/bin/env python3
"""Combine established QA with release gates for Articles 01–50."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output", type=Path,
        default=Path("results/articles-01-50-release-validation.json"),
    )
...
def main() -> int:
    ...
    chapters = manifest["series"]["chapters"][:50]
    components = {
    ...
    check(
        ...
        scaffold.get("pilot_articles") == list(range(1, 51)),
        scaffold.get("pilot_articles"),
    )
    check("manifest-formal-count", len(chapters) == 50, len(chapters))
    ...
    check("formal-html-count", html_count == 50, html_count)
    ...
    payload = {
        ...
        "scope": "Articles 01–50 local release assets",
