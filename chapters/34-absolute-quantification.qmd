...
reader-reproduction:
  ...
  script: examples/34-absolute-quantification.R
  packages:
  - ggplot2
  - ggrepel
  - knitr
...
[Vandeputte et al. (2017)](https://doi.org/10.1038/nature24460)把 flow-cytometry cell counts 与 16S composition 结合，展示 microbial load 如何改变 enterotype、疾病关联和共变网络的解释。以下使用其公开数据的 Bioconductor 分发版本，比较细胞负荷分布、相对与定量效应，以及一个两种尺度结论不一致的类群。
...
QMP 的估计同时包含组成测量误差和负荷测量误差。总负荷偏高会把同一样本中所有类群的定量值一起推高，所以这些误差不是彼此独立的。若负荷检测批次与病例分组重合，增加统计协变量也未必能分离真实负荷变化与测量偏差。应确认测量单位、取样质量或体积，以及负荷表与测序表的样本对应关系。

本例可比较同一队列的 29 个 CD 与 66 个 healthy 样本，但不能把得到的每克细胞数直接当作某菌的精确活菌数量。流式计数与序列比例的结合仍受提取、扩增和分类覆盖影响。若两种尺度方向相反，先把各自分母和总负荷差异展示清楚；这往往是生物学问题的关键，而不是需要删除的矛盾结果。
...
[下载完整 R 脚本](https://github.com/petemeng/microbiome-best-practices/blob/691722146e287afe063bd43e9ce4fa41977fe763/examples/34-absolute-quantification.R)。脚本包含数据下载、计算和图形导出。
...
## 比较相对变化和定量变化的方向 {#sec-publication}
...
## 报告微生物负荷与定量组成的方法与限制 {#sec-methods}
...
## 将微生物负荷与定量组成用于自己的研究 {#sec-own-data}
