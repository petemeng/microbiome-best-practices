...
原始 FASTQ 的质量合格，与拆样后每个样本得到可信读段，是不同层面的判断。前者可以发现文件损坏、编码或逐位置质量问题；后者还依赖 barcode 方向、样本映射和建库结构。文件级平均质量看起来正常时，少数样本仍可能没有正确分配到 reads。

先验证文件身份和配对关系，再用与交付格式匹配的入口导入，最后查看逐样本的拆样产量。若拆样率低，应首先检查 barcode 与建库约定，而不是立即放宽错配阈值；更宽松的匹配虽然可能增加分配数量，也可能增加误分配。FastQC 的 WARN 则需要按扩增子结构解释，不能把所有模块变成 PASS 当作分析目标。
...
::: {.callout-important title="文件正确与样本身份正确要分别证明"}
...
## 从质量分布判断是否可以进入去噪 {#sec-publication}
...
## 将数据导入、拆样与质量控制用于自己的研究 {#sec-own-data}
