---
title: "第 10 篇 · 导入数据 + provenance + 质控（FastQC/MultiQC）"
...
不要因为软件提供 Golay 纠错，就默认任意长度、任意设计的 index 都适用。若实验不是 EMP 12-nt Golay barcode，应依据建库协议选择对应 demultiplex 方法，不能只复制本页参数。
...
整仓库运行时可以改用 `source("R/theme_pub.R")`；单独复制分析时使用上面的内联定义。
