...
reader-reproduction:
  ...
  script: examples/38-community-assembly-bnti.R
  packages:
  - ape
  - ggplot2
  - iCAMP
...
### 过程比例的分母，为什么不是独立生态事件数？

βNTI–RCbray 分类的对象是样本对。同一个样本参与许多对比，因此把若干千对中的类别比例画成柱子，并不获得若干千个独立重复。比较组间比例的不确定性时，需要保留样本或地点层级，不能把所有 pair 当作独立伯努利试验。

本例先筛出 585 个较常见 OTU，估计的是这一保留集合及其区域类群库下的偏离。过滤更严格后，物种池、观测周转和零模型都会一起变化；这不是单纯提高运行速度。若系统发育信号缺乏支持，或过程比例随合理的 pool 定义显著改变，应停留在“相对零模型的周转差异”，不要继续给所有 pair 强行分配确定性生态机制。增加随机化次数只改善数值精度，不能修复错误的零模型。
...
[下载完整 R 脚本](https://github.com/petemeng/microbiome-best-practices/blob/691722146e287afe063bd43e9ce4fa41977fe763/examples/38-community-assembly-bnti.R)。脚本包含数据下载、计算和图形导出。
...
## 在系统发育假设下计算βNTI与RCbray {#sec-code}
...
### 正式运行 999 次 βNTI 与 RCbray null models
...
## 解释组装比例前先检查null model {#sec-publication}
...
## 报告群落构建模型与条件假设的方法与限制 {#sec-methods}
...
## 将群落构建模型与条件假设用于自己的研究 {#sec-own-data}
