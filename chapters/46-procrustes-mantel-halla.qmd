...
reader-reproduction:
  ...
  script: examples/46-procrustes-mantel-halla.R
  packages:
  - ggplot2
  - jsonlite
  - permute
...
[Franzosa et al. (2019)](https://doi.org/10.1038/s41564-018-0306-4)联合粪便宏基因组与代谢组刻画 IBD；公开整理仓库提供可追溯的配对 genus counts、metabolites 与 diagnosis metadata。
...
Control/CD/UC 调整后 Procrustes correlation 从 0.588 降到 0.529，说明部分整体相似性不只来自三个标签的均值差异；它并没有同时控制饮食、药物、批次或疾病活动度。诊断调整还改变了问题：从跨全部样本的共变，变成同诊断类别内部的共变。若诊断位于研究中的因果路径上，是否调整应由目标决定，而不是总把调整后的值称为“真实相关”。

整体检验与局部筛选也有不同分母。1,000 对候选关联在调整后从 437 个 FDR hits 降到 350 个，不能用仍有整体相关来替代每条边的检查。代谢物缺失可能受检测下限影响，菌的零值也有测量原因；两端共享缺失模式就可能产生关联。筛选出的 block 是后续验证的统计线索，不直接证明底物、产物或代谢方向。
...
[下载完整 R 脚本](https://github.com/petemeng/microbiome-best-practices/blob/691722146e287afe063bd43e9ce4fa41977fe763/examples/46-procrustes-mantel-halla.R)。脚本包含数据下载、计算和图形导出。
...
完整计算脚本：[run_article46_global_concordance.R](https://github.com/petemeng/microbiome-best-practices/blob/3cb6a817e0c73ab7ecbec1cedc1ad80cbb9ecfaa/scripts/run_article46_global_concordance.R)、[run_article46_halla.py](https://github.com/petemeng/microbiome-best-practices/blob/3cb6a817e0c73ab7ecbec1cedc1ad80cbb9ecfaa/scripts/run_article46_halla.py)。
...
## 报告跨组学关联与混杂调整的方法与限制 {#sec-methods}
...
## 将跨组学关联与混杂调整用于自己的研究 {#sec-own-data}
