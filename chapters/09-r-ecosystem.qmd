...
从三张表进入 R 分析时，要分别解决两个问题：软件能否运行，以及数据在对象转换后是否仍表示同一批样本。前者涉及 R、Bioconductor 和包之间的兼容性；后者涉及计数表方向、样本 ID、特征 ID，以及软件是否自动丢弃无法匹配的行列。

同一份 90 样本湿地计数表可以装入 `phyloseq` 或 `microtable`。这不是两次独立测量：如果转换正确，样本集合、特征集合和每个样本的总 reads 应保持一致。若后续两个包的结果不同，先排查默认过滤、归一化和距离参数，再讨论统计方法本身；仅凭对象维度相同不足以证明其中的数据一致。
...
::: {.callout-important title="对象转换不能悄悄改变样本和计数"}
...
## 检查对象转换和分析结果是否一致 {#sec-publication}
