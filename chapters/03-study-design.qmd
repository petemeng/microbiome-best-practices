...
reader-reproduction:
  ...
  script: examples/03-study-design.R
  packages:
  - ggplot2
  - lme4
  - permute
...
[下载完整 R 脚本](https://github.com/petemeng/microbiome-best-practices/blob/6c3ebab44401839f38b8d9356cb2f338ecfc9a3f/examples/03-study-design.R)。脚本包含数据下载、计算和图形导出。
...
### 更多样本解决精度问题，交叉对照解决可识别性问题

湿地数据的限制可以从模型写法看清：`outcome ~ Group + Salinity` 试图把同一组间差异拆成两个贡献，但 `Salinity` 完全由 `Group` 决定，数据没有提供拆分依据。将每组从 30 个样本增加到更多样本、同时保持这个布局，只会更精确地估计合并的组间差异；不会增加独立的盐度对照。

应把两个规划问题分开。首先检查主要比较在相关区组内是否有实际对照，以及模型矩阵是否满秩；其次才计算有意义效应所需的独立单位数。混合模型可以表示同一受试者或同一地点内的相关性，却不能把缺失的处理组合补出来。若预算有限，增加新的独立区组与增加现有区组内的样本，所增加的信息并不相同。

对于当前数据，可报告三类湿地之间的差异，也可把盐度作为组别描述；不能把调整后的盐度系数作为独立效应。若未来问题确实是盐度，则需要在可比湿地或区组内获得盐度变化，并记录决定取样位置的因素。设计改变后，样本量才应按新的主要比较重新计算。
...
## 报告研究设计与样本量依据的方法与限制 {#sec-methods}
...
## 将研究设计与样本量依据用于自己的研究 {#sec-own-data}
