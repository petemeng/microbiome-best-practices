...
> **分析目标：**从 SILVA 138.2 完整 SSU 发布提取与 515F/806R 一致的 V4 参考区，训练当前 QIIME 2 环境可读取的 Naive Bayes 分类器；用不泄漏序列的 known-lineage 留出集比较 confidence 0.50、0.70、0.90，再用全部参考重训生产模型。  
> **输入文件：**SILVA 138.2 SSURef NR99 V4 exact-sequence LCA 参考 FASTA 与 taxonomy、366 条真实 V4 ASV、对应 5,213 条 reads 的频数。换成自己的数据时，还需标准 `otutab.tsv`、`metadata.tsv` 和与 ASV ID 对齐的代表序列 FASTA；本篇生成新的 `taxonomy.tsv`。  
> **预计资源：**本次 262,752 条区域参考在 4 个分类线程下，两次训练约 13.2 分钟；导入、三档留出分类、三档真实 ASV 分类、maximum validation 与审计合计约 30–40 分钟。建议预留 16 GB 内存和 2 GB 工作空间。  
> **执行策略：**参考拆分、两次训练、六次分类、QIIME taxonomy evaluation、Artifact validation 与 provenance 已一次性实跑；网页构建保留 `eval: false`，避免每次渲染重新训练模型。
...
### 5. precision、recall、F1 与 abstention 要一起看
...
### 8. classifier 与 scikit-learn 版本是一组合同
...
<details>
<summary><strong>展开：固定环境、真实数据、checksum 与内联作图函数</strong></summary>
...
### 5. 用全部 reference 重训生产 classifier
...
### 7. 在本篇内重算 sequence-search comparator
...
### 1. 验证主图同时放 precision、recall 和 F1
...
### 2. 正式样本用 read-weighted coverage 展示阈值代价
...
### 3. 方法比较图必须把 coverage 与 agreement 分开
