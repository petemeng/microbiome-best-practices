...
> **分析目标：**从真实 16S counts 推断属级 association network，对比 SparCC marginal correlations 与 SPIEC-EASI sparse conditional associations，审计 permutation resolution、StARS stability、方法重叠和 group-driven edges；ggClusterNet 作为布局/工作流层讨论，不把漂亮网络当作交互证据。  
> **输入文件：**`data/small/otutab.tsv`、`taxonomy.tsv`、`metadata.tsv`。  
> **预计资源：**90 个样本、40 个 group-blind 高信息属；SparCC 200 次 bootstrap/permutation + SPIEC-EASI 50 次 StARS subsampling，单线程通常 4–8 分钟、峰值内存约 2–4 GB；三张图导出为 PDF/SVG/600 ppi PNG/LZW-TIFF。

## 共现网络不是微生物互作网络 {#sec-target}

[Friedman & Alm (2012)](https://doi.org/10.1371/journal.pcbi.1002687)提出 SparCC，用 composition-aware correlation 重建 microbial association network；[Kurtz et al. (2015)](https://doi.org/10.1371/journal.pcbi.1004226)提出 SPIEC-EASI，以 sparse inverse-covariance/neighborhood selection 估计条件关联。两篇 PLOS 原文为开放许可。这里不复制原图，而是用真实湿地 16S 三件套重绘 correlation matrix、SPIEC-EASI network 和 edge audit ledger。
...
## 理论：相关、条件关联和生态作用是三层证据 {#sec-theory}

### 1. Ordinary correlation 会被 closure 扭曲
...
### 2. SparCC edge 是 marginal association
...
### 3. SPIEC-EASI edge 是 sparse conditional association
...
### 4. Pooling groups 可制造 Simpson's paradox
...
### 5. ggClusterNet 是工作流与布局层，不是独立真值层
...
本页实跑 `SpiecEasi` 1.1.2（官方仓库 commit `c463727`）。ggClusterNet 未进入主计算依赖；其 GitHub master 会变化，正式项目应把安装当日 SHA 写进 lockfile。整仓库使用者可改用 `source("R/theme_pub.R")`；单篇复现直接运行下面定义：
...
### 2. SparCC + 200 次 bootstrap/permutation
...
### 图 1：SparCC correlation matrix 叠加 SPIEC-EASI edges
...
### 图 2：固定 seed 的 SPIEC-EASI association network
...
### 图 3：边数、方法交集和 group sensitivity 进入同一账本
