...
reader-reproduction:
  ...
  script: examples/28-core-rare-biosphere.R
  packages:
  - ggplot2
  - ggrepel
  - ragg
...
本页使用 `microeco` 2.0.0 随包分发的中国湿地土壤 16S 数据。原研究 [An et al. (2019)](https://doi.org/10.1016/j.geoderma.2018.09.035)比较 inland、coastal 与 Tibetan wetland 的细菌群落；[Liu et al. (2021)](https://doi.org/10.1093/femsec/fiaa255)提供了可复现的三件套数据入口。分析同时比较检出频率与相对丰度，避免将常见和优势混为一谈。
...
本例 97 个 global core 中有 34 个的平均相对丰度低于 0.1%，所以“经常检出”和“占据大量 reads”是两个维度。更重要的是，横断面中 80% 的样本检出，描述的是这个样本集合的占据率，不是同一个体在 80% 的时间持续携带。稳定定植需要有时间顺序的重复采样，并区分重复检出与反复输入。

一次未检出也未必意味着不存在：低深度下，稀有 feature 更容易得到零。只有出现阈值、丰度阈值和样本集合都固定，core 集合才可比较。这里将阈值从 presence 改为 0.01% 就从 334 个降至 97 个；这项变化应成为结果的一部分。若研究需要一个可推广的 core 清单，还要在独立队列按同一规则估计检出率及其区间，而不是在训练样本上不断调阈值直到清单大小合意。
...
[下载完整 R 脚本](https://github.com/petemeng/microbiome-best-practices/blob/691722146e287afe063bd43e9ce4fa41977fe763/examples/28-core-rare-biosphere.R)。脚本包含数据下载、计算和图形导出。
...
### 同时扫描 detection、prevalence 与 read depth
...
## 比较核心定义对阈值和测序深度的敏感性 {#sec-publication}
...
## 报告核心菌群与稀有类群定义的方法与限制 {#sec-methods}
...
## 将核心菌群与稀有类群定义用于自己的研究 {#sec-own-data}
