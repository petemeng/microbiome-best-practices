...
一个可以打印版本号的 QIIME 2 环境，仍可能缺少插件、读错文件格式，或者使用了与数据不符的 semantic type。判断它能否用于分析，需要让真实输入走完“导入 → 校验 → 导出”的往返，再把导出文件与原始输入比较。这样能把安装问题和数据问题分开，而不会在每次报错后重装所有软件。

这个往返测试也有明确的边界：它证明文件及其类型能够被正确读取和保存，不证明样本标签、生物学分组或后续去噪参数正确。环境可用之后，仍需按实验设计检查 metadata；不能把成功生成 `.qza` 当作数据已经完成质控。
...
::: {.callout-important title="验证环境要走完一次真实数据往返"}
