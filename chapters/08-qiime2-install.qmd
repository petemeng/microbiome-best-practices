---
title: "第 08 篇 · 安装 QIIME2 + 验证 + 报错对照"
...
> **分析目标：**安装版本固定的 QIIME 2 2026.4 独立环境，并用真实 Atacama multiplexed paired-end FASTQ 完成 `import → validate → peek → export` 闭环；同时建立按失败层定位的报错对照。  
> **输入文件：**`env/qiime2.yml`，以及 `data/small/fastq/` 中的 `forward.fastq.gz`、`reverse.fastq.gz`、`barcodes.fastq.gz`。  
> **预计资源：**环境约占 5.1 GB；首次求解与下载通常 20–60 分钟。本篇 2,000-record 真实数据验收约 4 分钟、峰值内存低于 2 GB，生成的 `.qza` 约 0.5 MB。  
> **执行策略：**安装与命令行验收已经一次性实跑并保存日志；网页渲染使用 `eval: false`，避免每次构建都重新创建环境或生成新的 Artifact UUID。
...
## 理论：QIIME 2 的“安装成功”究竟指什么 {#sec-theory}

### 1. 安装的是 distribution，不是一个孤立 Python 包
...
### 2. release、软件版本与环境 manifest 是三层信息
...
### 5. Artifact 保存数据、类型、校验与 provenance
...
<details>
<summary><strong>展开：系统条件、真实数据、环境文件与出版级函数</strong></summary>
...
没有整仓库文件时，可以按 QIIME 2 Library 当前 release 的 Linux/WSL2 命令直接创建官方环境：
...
整仓库的验收器会创建临时 EMP 目录、设置可写缓存、完成真实 import/validate/export、扫描 archive，并执行四个预期失败探针：
