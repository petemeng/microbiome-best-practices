...
长度差异会改变哪些分子能够进入最终特征表。本例提取出的 ITS2 ASV 长度为 143–270 nt；如果不看长度分布就强制截成同一长度，丢失的可能不只是低质量读段，还包括某些类群的真实长片段。应分别检查目标区域提取、质量过滤和双端合并造成的损失，再判断是否需要单端分析；单端结果也必须重新确认可用的分类分辨率。[ITSxpress 方法论文](https://pmc.ncbi.nlm.nih.gov/articles/PMC6206612/)解释了为什么应先识别 ITS 边界，而不是把保守侧翼与 spacer 混在一起分析。

不同 marker 的相对丰度也没有共同分母。16S 表中的 20% 与 ITS 表中的 20% 不能相加，更不能直接解释为细菌与真菌生物量各占多少；引物效率、拷贝数和参考覆盖都不同。联合分析应先在各自的测量范围内得到结果，再按共同样本比较其关联。
...
::: {.callout-important title="按 marker 决定长度处理和参考范围"}
...
## ITS 与 18S 为什么不能照搬 16S 参数 {#sec-theory}
...
### 用 ITSxpress 提取 ITS2，保留未合并 pairs
...
## 判断序列、数据库和标记区域是否匹配 {#sec-publication}
...
## 将ITS与18S分析用于自己的研究 {#sec-own-data}
