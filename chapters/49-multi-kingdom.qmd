...
### 配对完整之后，为什么还不能直接比较两界的丰度？

同一样本有三种 marker，只保证比较对象配对，不保证测量单位相同。各界的提取、引物和拷贝数不同，分别闭合后的两个 10% 不能相减来判断哪一界细胞更多。联合分析应保留各自的特征空间，在样本层比较结构；需要生物量比较时还要有匹配的独立定量。

本例调整 soil × compartment 后，局部 FDR hits 从 505 降到 2，表明大部分原始相关可以由共同生态位解释。剩余两对的高 bootstrap 符号稳定性只针对当前分层重采样，不排除未测环境或宿主因素。每个 niche cell 的独立重复有限，不能把多组学测量次数算作新增生物重复；两条边应作为待验证关联，而不是相互作用机制。
