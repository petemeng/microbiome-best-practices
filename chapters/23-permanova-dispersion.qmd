...
reader-reproduction:
  ...
  script: examples/23-permanova-dispersion.R
  packages:
  - digest
  - ggplot2
  - jsonlite
...
原研究 Figure 2 的预处理保留“在任一处理至少 4 个 tagged libraries 中检出”的 feature。以下沿用该阈值作为主分析，先把同一土壤样本的 1–3 个技术文库相加，再计算 relative-abundance Bray–Curtis；四张图分别呈现实验单位、中心与离散度、计划比较和 PERMDISP 结果。
...
### PERMDISP 不显著，为什么仍然不能宣布检验安全？

离散度检验未拒绝原假设，不等于证实各组离散度相等；它也可能缺少识别差异的精度。反过来，两个检验都显著，也不能机械地说 PERMANOVA 全部无效：位置与离散度差异可以同时存在，影响还取决于样本量平衡和研究设计。应同时看组内距离分布、样本量、效应量和合法置换，而不是把 PERMDISP 当成一个通过后即可忽略其他假设的开关。[vegan 方法说明](https://vegandevs.github.io/vegan/reference/adonis.html)明确区分了两类检验。

本例更大的不稳定性来自特征集合：保留全部 feature 后 P 从 0.0065 变为 0.7642。主过滤规则虽有研究依据，仍使用了处理内检出信息；这种条件应在结论中保留。若新研究需要确认性检验，更容易解释的做法是提前固定不依赖待检验组标签的过滤规则，再把其他合理规则作为敏感性分析，不能在看过各分支 P 值后选择主结果。
...
[下载完整 R 脚本](https://github.com/petemeng/microbiome-best-practices/blob/691722146e287afe063bd43e9ce4fa41977fe763/examples/23-permanova-dispersion.R)。脚本包含数据下载、计算和图形导出。
...
### 必配 PERMDISP，并保留 ANOSIM/MRPP 的方向
...
## 把设计、效应量和离散度放在一起解释 {#sec-publication}
...
## 将置换设计与群落差异用于自己的研究 {#sec-own-data}
