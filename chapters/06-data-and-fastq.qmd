---
title: "第 06 篇 · 认识你的数据：测序原理 + FASTQ / 文件格式拆解"
...
> **分析目标：**从一组真实的 multiplexed paired-end FASTQ 出发，读懂 read header、碱基序列、Phred 质量、R1/R2/index 的对应关系，并建立从原始 reads 到 `otutab` / `taxonomy` / `metadata` 三件套的文件地图。  
> **输入文件：**`forward.fastq.gz`、`reverse.fastq.gz`、`barcodes.fastq.gz`、`metadata.tsv`。  
> **预计资源：**本篇随附 2,000 组真实同步 reads，普通笔记本约 20–40 秒、内存低于 300 MB；官方 1% 完整数据为 135,487 组 reads。
...
完整 URL、完整校验值、2,000-record 摘录的生成规则和摘录文件校验值均保存在 `data/small/fastq/source_summary.json`。本篇的图是根据真实 reads 重新计算的原创图，不复制论文图版。

## 理论：从荧光信号到四行文本 {#sec-theory}

### 1. paired-end 测序得到的是两个方向的观测
...
### 4. multiplexed 与 demultiplexed 是两种文件合同
...
本篇使用 R 4.4.1、`ggplot2 3.5.2`、`dplyr 1.1.4`、`tidyr 1.3.1`、`jsonlite 1.8.8` 与 `digest 0.6.36`。首次运行安装：
...
```{bash}
#| eval: false
gzip -t data/atacama-1p/forward.fastq.gz
gzip -t data/atacama-1p/reverse.fastq.gz
gzip -t data/atacama-1p/barcodes.fastq.gz

sha256sum data/atacama-1p/*
```
...
### 图 2：FASTQ anatomy 使用真实 record，但避免长字符串挤压
...
- `file contract` 回答 FASTQ、QIIME 2 artifact 与 R 三件套如何衔接。
:::
