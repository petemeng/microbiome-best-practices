...
> **分析目标：**在同一真实 16S 数据集上分别估计 standardized Levins sample-use breadth、逐样本 rank-abundance distribution（RAD）模型与 Bray–Curtis geographic distance-decay，并把三类描述量的解释边界分开。  
> **输入文件：**通用三件套 `data/small/otutab.tsv`、`taxonomy.tsv`、`metadata.tsv`；距离衰减额外读取同一数据源的 `environment.tsv` 中 `Latitude`/`Longitude`。  
> **预计资源：**13,628 个 OTU、90 个样本；90 × 5 个 RAD fits、999 次分层 Mantel permutation，通常 2–5 分钟、峰值内存低于 2 GB；四张图导出为 PDF/SVG/600 ppi PNG/LZW-TIFF。

## 生态位、丰度分布与距离衰减回答不同尺度问题 {#sec-target}

[Martiny et al. (2011)](https://doi.org/10.1073/pnas.1016308108)展示了 bacterial beta-diversity 的空间尺度依赖；[Hanson et al. (2012)](https://doi.org/10.1038/nrmicro2795)系统梳理了 selection、dispersal、drift 与 diversification 如何共同塑造微生物生物地理格局。这里不复制论文原图，而是在公开湿地 16S 数据上重绘 breadth–occupancy、rank-abundance fit、within-region distance-decay 和 effect-size audit。
...
## 理论：三种宏生态图不能互相代替 {#sec-theory}

### 1. standardized Levins breadth 是 sample-use evenness
...
### 2. RAD 比较的是同一样本内的 abundance hierarchy
...
### 4. pooled slope 容易被 region separation 劫持
...
### 2. standardized Levins sample-use breadth
...
### 4. Haversine distance、within-region slope 与 Mantel audit
...
### 图 1：sample-use breadth 与 occupancy
