...
reader-reproduction:
  ...
  script: examples/02-scope-and-limits.R
  packages:
  - ggplot2
  - scales
---
...
[下载完整 R 脚本](https://github.com/petemeng/microbiome-best-practices/blob/6c3ebab44401839f38b8d9356cb2f338ecfc9a3f/examples/02-scope-and-limits.R)。脚本包含数据下载、计算和图形导出。
...
### 测序更深，为什么不能自动消除提取与扩增偏倚？

可以把一个类群的预期 reads 比例理解为“真实数量 × 流程检出效率”，再除以所有类群对应量的总和。增加测序量主要减少从这一分布抽样时的随机误差；它不会把检出效率自动变成 1。因此，一个低丰度类群可能需要更深测序才能稳定检出，而一个始终难以裂解或扩增的类群可能在每次深测序中都被低估。[McLaren 等的已知组成实验](https://elifesciences.org/articles/46923)支持用类群和流程相关的乘性偏倚理解这种现象。

这会改变技术选择：若主要疑问是漏检，检查深度和检测概率；若主要疑问是定量偏差，增加覆盖相关类群和处理步骤的对照，而不是只增加 reads。若两组恰好使用不同提取或引物流程，生物差异与检出效率变化还会混在一起。当前湿地 taxonomy 表只能检查注释标签的覆盖，既不能校准这些上游偏倚，也不能证明未注释类群在样本中不存在。
