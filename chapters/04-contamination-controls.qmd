---
title: "污染与对照：blank、kitome、低生物量、decontam"
author: "Peter"
date: last-modified
format:
  html:
...
reader-reproduction:
  ...
  script: examples/04-contamination-controls.R
  packages:
  - decontam
  - ggplot2
  - scales
...
## 低生物量研究为什么必须先证明信号不是污染？ {#sec-target}
...
[下载完整 R 脚本](https://github.com/petemeng/microbiome-best-practices/blob/6c3ebab44401839f38b8d9356cb2f338ecfc9a3f/examples/04-contamination-controls.R)。脚本包含数据下载、计算和图形导出。
...
### frequency 与 prevalence 不一致时，先检查它们各自依赖什么

两种结果不一致，不是让分析者投票选一个。frequency 依赖浓度与污染相对比例的关系；如果总 DNA 主要来自宿主，浓度可能不是微生物生物量的良好代理。prevalence 依赖阴性对照是否覆盖了真正发生污染的流程和批次；某块板没有对照时，其他板的 blank 不能直接替它提供同等证据。这些是模型输入是否有意义的问题，不能只靠改变分类阈值解决。[decontam 原始研究](https://pmc.ncbi.nlm.nih.gov/articles/PMC6298009/)分别定义了这两类证据及所需辅助数据。

在本例每板 5 个阴性对照的设计中，先比较同板对照与样本，再看合并分析是否改变候选名单。一个 ASV 同时出现在 blank 和样本中，并不能区分“纯污染”“真实成员叠加污染”或“串样”。因而特征剔除是有代价的分析选择：应同时查看候选污染物对每个样本的贡献，以及剔除前后主要生物学结论是否改变。

若结论只在某一个阈值、某一种对照合并方式下成立，应把它写成对污染处理敏感；不要以“删除了更多 ASV”作为处理更成功的标准。缺少合适对照时，能做的是限制结论并补充证据，不能把分类器输出当作已经验证的样本无污染证明。

## 利用阴性对照与DNA浓度识别候选污染 {#sec-code}
...
## 将对照设置与污染判定用于自己的研究 {#sec-own-data}
