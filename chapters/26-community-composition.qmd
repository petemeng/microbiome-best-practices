...
### 同一组样本，为什么两种“平均组成”会不同？

先合并一组的所有 reads 再算比例，会让测得更深的样本权重更高；先算每个样本的比例再取均值，则让每个样本等权。两者不是可以随意互换的作图技巧。如果研究对象是典型样本的组成，本例采用后者；如果要描述合并测序池，前者才对应那个目标，但它仍不等于按生物量加权的群落。

Top 10 也要在同一候选集合上选。若每组各选自己的 Top 10，再把剩余部分都叫 Other，相同颜色就可能代表不同的类群集合。下面始终使用全样本共同的 Top 10，并保留 Unknown；属水平注释较少时，读者因此可以分清“这组真的缺少某属”和“这组有更多 reads 无法命名”。热图的行 Z-score 另外只表达某属在样本间的相对高低，不能用于比较不同属的绝对优势程度。
