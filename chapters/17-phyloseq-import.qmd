...
reader-reproduction:
  ...
  script: examples/17-phyloseq-import.R
  packages:
  - Biostrings
  - ape
  - digest
...
`phyloseq` 对象通常不直接成为论文结果图，却是群落组成、alpha diversity、beta diversity、差异丰度和系统发育分析共同的数据入口。McMurdie 与 Holmes 在 `phyloseq` 方法论文中用对象与分析组件的关系说明这种设计。面对真实输入时，先检查以下四件事：
...
[下载完整 R 脚本](https://github.com/petemeng/microbiome-best-practices/blob/691722146e287afe063bd43e9ce4fa41977fe763/examples/17-phyloseq-import.R)。脚本包含数据下载、计算和图形导出。
...
## phyloseq 如何保持特征、分类和样本一致 {#sec-theory}
...
### 完整 reference-placement tree 要先解释、再裁剪
...
完整计算脚本：[validate_phyloseq_import.R](https://github.com/petemeng/microbiome-best-practices/blob/3cb6a817e0c73ab7ecbec1cedc1ad80cbb9ecfaa/scripts/validate_phyloseq_import.R)。
...
## 检查对象构建前后的样本和特征 {#sec-visualization}
...
## 报告phyloseq对象构建的方法与限制 {#sec-methods}
...
## 将phyloseq对象构建用于自己的研究 {#sec-own-data}
