...
reader-reproduction:
  ...
  script: examples/45-survival-analysis.R
  packages:
  - ggplot2
  - glmnet
  - ragg
...
[Zhang et al. (2018)](https://doi.org/10.1038/s41564-018-0251-7)研究 early-life antibiotics、gut microbiome 与 NOD mice type 1 diabetes development；[Gu et al. (2023)](https://doi.org/10.1128/spectrum.05059-22)基于该公开数据提出 MiSurv。以下分析使用 6–8-week baseline 16S tables，依次检查 treatment KM、adjusted genus HR、PH diagnostic 与 锁定测试集 time-dependent ROC。
...
### 时间起点选对之后，还要检查谁能进入风险集

以 6–8 周采样时作为预测起点，结论适用于当时仍处于风险中且获得样本的小鼠，不自动适用于出生时的整个队列。若从出生计时却使用以后才采到的微生物特征，就需要正确处理延迟入组或采用明确的 landmark 设计；把采样后的信息当成出生时已知会引入偏差。

删失也不是“没有事件”的另一种写法。Cox 和 KM 的解释需要相应的非信息删失假设；若失访与未测风险有关，普通模型可能有偏。模型复杂度应看实际事件数而不只看样本量，HR 是条件瞬时风险比，也不是固定时点的发病概率比。时间依赖 ROC 应连同评估时点、仍在随访人数和删失处理报告，不能从多个时点中只选择最高 AUC。
...
[下载完整 R 脚本](https://github.com/petemeng/microbiome-best-practices/blob/691722146e287afe063bd43e9ce4fa41977fe763/examples/45-survival-analysis.R)。脚本包含数据下载、计算和图形导出。
...
### 预定义 Treatment 的 Kaplan–Meier 描述
...
## 同时阅读生存曲线、风险比和预测误差 {#sec-beautify}
...
## 将生存结局与删失处理用于自己的研究 {#sec-own-data}
