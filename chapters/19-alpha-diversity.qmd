...
reader-reproduction:
  ...
  script: examples/19-alpha-diversity.R
  packages:
  - dplyr
  - ggplot2
  - iNEXT
...
本页使用 An 等中国湿地研究随 `microeco 2.0.0` 分发的真实 16S 数据：13,628 个 OTU、90 个样本、1,619,670 条 reads，内陆湿地、沿海湿地和青藏高原湿地各 30 个样本。[原始数据论文](https://doi.org/10.1016/j.geoderma.2018.09.035)与 [`microeco` 软件论文](https://doi.org/10.1093/femsec/fiaa255)提供了可引用的数据来源。
...
### 相同 coverage 之后，哪些不确定性仍然存在？

90% coverage 不是“测到了 90% 的物种”，而是对当前群落中已观测类群所覆盖的丰度质量的估计。它让不同样本在更接近的采样完整度上比较，但不能修复引物偏倚、把 OTU 变成真正的物种，或把地点内的重复样本变成独立重复。本例对 34 个样本进行外推，所以还应查看这些样本的区间，而不能把外推后的点估计视为新增的观测 reads。

这里有两层不同的不确定性：给定一个样本时，由有限 reads 带来的多样性估计误差；跨生物学样本时，由个体或地点差异带来的组间估计误差。iNEXT 曲线区间主要处理前者，组间模型与按独立单位的重采样处理后者。若主要结论只在高度外推的 richness 上出现，应降低外推目标或增加采样证据，而不是更换指数寻找显著性。

## 下载并读取本次分析的数据 {#sec-preparation}
...
[下载完整 R 脚本](https://github.com/petemeng/microbiome-best-practices/blob/691722146e287afe063bd43e9ce4fa41977fe763/examples/19-alpha-diversity.R)。脚本包含数据下载、计算和图形导出。
...
### Richness、evenness 和 dominance 不是同一个问题
...
## 从测序深度比较到相同完整度 {#sec-code}

### 在未过滤计数上计算完整 Alpha 指数表
...
## 用稀释曲线与Hill profile解释多样性 {#sec-publication}
...
## 将多样性指数与标准化用于自己的研究 {#sec-own-data}
...
### Coverage 目标也要做 sensitivity analysis
