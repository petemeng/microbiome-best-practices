...
两张因果图可能蕴含相同或非常接近的条件独立关系，因而在横断面数据上都拟合良好。箭头从 Shannon 指向 TOC，是预设机制假设，不是回归自己识别出的时间方向；TOC 也可能影响群落，或二者共享未测原因。应把竞争解释与研究能测到的时间信息并列，而不是只给最合意的一张图。

标准化系数以当前样本的标准差为单位，换一个环境范围或人群后大小可能改变；它不是跨研究固定的机制强度。若根据同一数据逐条添加路径直到 Fisher's C 不显著，最终检验已包含探索性选择。应区分预设结构与事后修改，并在独立数据检查后者。增加更多微生物指标也会增加参数负担，不能用复杂路径图替代缺失的测量与设计。
