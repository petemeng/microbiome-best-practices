...
> **分析目标：**用预先声明的分段结构方程模型（piecewise SEM）检验“气候 → 土壤化学 → 微生物多样性 → 土壤有机碳”路径与数据是否相容，并用 PLS-PM 做估计器敏感性分析。  
> **输入文件：**`otutab.tsv`、`taxonomy.tsv`、`metadata.tsv`，以及逐样本实测的 `environment.tsv`；后者必须包含环境因子和表型，不能由 OTU 表虚构。  
> **预计资源：**真实示例为 13,628 个 OTU、90 个样本、4 个实测环境变量和 1 个实测表型；普通笔记本约 3–8 分钟，内存低于 1 GB。
...
本例使用中国湿地土壤公开 16S 数据。原研究覆盖内陆、沿海和青藏高原湿地，并测量 pH、电导率、温度、降水和总有机碳（TOC）；`microeco` 以标准 OTU、分类、样本和环境表分发其中的示例。[数据研究（An et al., 2019）](https://doi.org/10.1016/j.geoderma.2018.09.035)与[`microeco` 软件论文](https://doi.org/10.1093/femsec/fiaa255)提供出处。

这里重绘自己的路径图，不复制论文原图。分析前把结构写成：气候可直接影响土壤化学、Shannon 和 TOC；土壤化学可影响 Shannon 与 TOC；Shannon 可影响 TOC；湿地组别进入所有分量模型作为调整变量。
...
数据固定使用 `microeco 2.0.0` 源码包中的真实表。下面下载、核对 SHA-256，并导出三件套和环境表。
...
tmp <- tempfile("microeco-")
dir.create(tmp)
utils::untar(archive, files = files, exdir = tmp)
e <- new.env(parent = emptyenv())
for (f in files) load(file.path(tmp, f), envir = e)
...
write_piece(e$otu_table_16S, "FeatureID", "data/small/otutab.tsv")
write_piece(e$taxonomy_table_16S, "FeatureID", "data/small/taxonomy.tsv")
write_piece(e$sample_info_16S, "SampleID", "data/small/metadata.tsv")
write_piece(e$env_data_16S, "SampleID", "data/small/environment.tsv")
...
整套项目可使用内容相同的 `R/theme_pub.R`；单独复制文章时保留上面的函数。
