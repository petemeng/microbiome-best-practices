...
### 属水平图上的组间差异，会不会来自注释覆盖不同？

三组能命名到属的 reads 比例分别为 28.99%、38.38% 和 43.31%。若只对已知属重新闭合到 100%，比较的问题就变成“在各组可命名的部分里，某属占多少”，而不再是全部群落中的份额。这个条件分母不同，可能放大原本很小的视觉差异。因此主图保留全部 reads 分母，已知属内部的组成只能作为明确标注的补充视图。

Family→Genus 损失 31.07 个百分点，也说明更细的面板未必提供更多可靠信息。若关键结论在 family 层稳定、genus 层只剩少量可注释 reads，可以把 family 作为主要报告分辨率，同时呈现 genus 的有限证据；不应为了得到更具体的名字删除未知部分。跨级别的同名标签还要保留父系路径，否则会把不同谱系错误合并。
