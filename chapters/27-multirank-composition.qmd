...
> **分析目标：**从同一张 feature count table 同时生成 Phylum、Class、Order、Family、Genus 五级组成结果，并让 rank、lineage、缺失父级、Top-N 与分母都可审计。  
> **输入文件：**`otutab.tsv`、`taxonomy.tsv`、`metadata.tsv`。  
> **预计资源：**示例含 13,628 个 OTU、90 个样本和 1,619,670 reads；五级聚合、完整 lineage 审计与四张 600 ppi 图通常需 1–3 分钟，峰值内存低于 1 GB。
...
## 理论：多分类级不是把五张表直接拼起来 {#sec-theory}

### 1. 每个 rank 都有自己的 100%，跨 rank 不能相加
...
### 2. 逐 rank 聚合，并保留 rank-qualified label
...
### 5. 固定每级 Top 5，并保留 Top-N coverage 敏感性
...
### 6. 把 pooled reads 和 known-only 留在敏感性分支
...
### 7. 选择完整优势 lineage，而不是孤立 genus 名
...
## 出版级美化：四张图各回答一个层级问题 {#sec-publication}

### 1. 注释分辨率阶梯：先看每一级还有多少 reads 可命名
