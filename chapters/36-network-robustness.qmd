...
[Guimerà & Amaral (2005)](https://doi.org/10.1038/nature03288)用 within-module degree 与 participation coefficient 建立网络角色图；[Yuan et al. (2021)](https://doi.org/10.1038/s41558-021-00989-9)把网络复杂度与稳定性用于土壤微生物群落比较。这里不复制论文原图，而是在公开湿地 16S 数据上重绘 role cartography、node-attack curve、group rewiring null distribution 和方法敏感性账本。
...
## 理论：拓扑角色、鲁棒性和组间差异是三个问题 {#sec-theory}
...
本页实跑 `SpiecEasi` 1.1.2（官方仓库 commit `c463727`）。整仓库使用者可改用 `source("R/theme_pub.R")`；单篇复现直接运行下面定义：
