...
这里不复制该人群研究的数值或原图，而是使用 `microeco` 2.0.0 公开分发的中国湿地土壤 16S 数据重新绘图。原始数据研究为 [An et al. (2019)](https://doi.org/10.1016/j.geoderma.2018.09.035)，包含 90 个可与环境表匹配的样本；`microeco` 的数据与软件说明见 [Liu et al. (2021)](https://doi.org/10.1093/femsec/fiaa255)。
...
若在完整仓库中运行，也可把上面的函数块替换为 `source("R/theme_pub.R")`；正文复现不依赖该文件。
