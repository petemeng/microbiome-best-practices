...
reader-reproduction:
  ...
  script: examples/25-environment-variance.R
  packages:
  - ggplot2
  - jsonlite
  - permute
...
同一批样本上的 envfit、Mantel 和 VPA 共享群落数据与环境梯度，结果一致只能说明不同摘要方式给出相容的模式，不能当作三个独立队列的复现。本例 Temperature 与 Latitude 的二维拟合强度几乎相同，正提醒我们相关环境变量可能承载同一地理梯度。

90 个样本能产生 4,005 个样本对，但 Mantel 的独立信息并没有因此变成 4,005 份。每个样本参与多个距离，置换必须作用于样本标签，并保持设计允许的交换关系。若群落与环境都存在空间自相关，自由置换可能破坏各自的空间结构；应先明确空间采样与目标零假设，不能把 partial Mantel 当作自动控制空间混杂的办法。[Mantel 方法说明](https://vegandevs.github.io/vegan/reference/mantel.html)也强调距离关联与普通独立观测回归并不相同。

VPA 的 7.1% 共享部分同样不能被唯一归给环境或地理。若希望估计可迁移的环境响应，需要新的组内环境变化或独立地点验证，而不是进一步拆分同一份共享百分比。
...
[下载完整 R 脚本](https://github.com/petemeng/microbiome-best-practices/blob/691722146e287afe063bd43e9ce4fa41977fe763/examples/25-environment-variance.R)。脚本包含数据下载、计算和图形导出。
...
### 检查 VIF、conditioning 和 prevalence 敏感性
...
## 将环境关联与变异分解用于自己的研究 {#sec-own-data}
