...
reader-reproduction:
  ...
  script: examples/33-da-visualization.R
  packages:
  - ANCOMBC
  - S4Vectors
...
[Segata et al. (2011)](https://doi.org/10.1186/gb-2011-12-6-r60)用 LDA bar plot 与 cladogram 展示跨 taxonomy 层级 biomarker；现代微生物组论文还常用 volcano、按 phylum 分块的 Manhattan 和 effect-size forest plot。以下用湿地数据比较四类图各自适合回答什么问题。它们共享一份 master table，差别只在视觉任务：筛查全局、定位 lineage、观察 taxonomy block、读取效应与区间。
...
### 为什么 q 值不显著，95% 区间却可能不跨零？

普通逐项 95% Wald 区间对应单个系数的近似推断；BH q 值则考虑整组检验。两者不一定给出相同的二元判断，图中应分别标明区间类型和显著性规则，不能为了统一颜色把区间裁到零线外。若需要同时覆盖整组效应，应使用相应的同时推断方法，而不是给普通区间换名称。

只画筛选后的最强信号还会夸大效应的典型大小。Volcano 和 Manhattan 应保留完整受检背景；forest 可以突出预先关心的菌，但需明确选择规则并附原始分布。很大的系数配很宽的区间通常提示信息不足或稀疏，而不是一个已确定的强效应。LEfSe LDA score 与 ANCOM-BC2 系数使用不同尺度，不能共用一条效应轴比较大小。

## 下载并读取本次分析的数据 {#sec-setup}

需要 R，以及 `ANCOMBC`、`S4Vectors`、`SummarizedExperiment`、`ggplot2`、`ggraph`、`ggrepel`、`igraph`、`knitr`、`lefser`、`phyloseq`、`ragg`、`readr`、`svglite`。本次使用中国湿地的 OTU count 表、分类表和样本信息表。
...
[下载完整 R 脚本](https://github.com/petemeng/microbiome-best-practices/blob/691722146e287afe063bd43e9ce4fa41977fe763/examples/33-da-visualization.R)。脚本包含数据下载、计算和图形导出。
...
### Volcano：全表筛查，不是 effect uncertainty 图
...
### Cladogram：taxonomy context，不是系统发育树
...
## 用火山图定位，再用森林图定量解释 {#sec-publication}
...
## 报告差异图的效应与筛选规则的方法与限制 {#sec-methods}
...
## 将差异图的效应与筛选规则用于自己的研究 {#sec-own-data}
