...
reader-reproduction:
  ...
  script: examples/05-batch-effects.R
  packages:
  - MMUPHin
  - decontam
  - ggplot2
...
[下载完整 R 脚本](https://github.com/petemeng/microbiome-best-practices/blob/6c3ebab44401839f38b8d9356cb2f338ecfc9a3f/examples/05-batch-effects.R)。脚本包含数据下载、计算和图形导出。
...
### 校正后混在一起，不等于主要效应被保留下来

校正算法可以降低板间差异，也可能同时降低与板相关的真实生物差异。只看排序图是否混合，无法区分这两种情况。更有信息的比较是在相同样本、相同特征和相同主要 contrast 下，观察效应方向、大小及区间是否稳定；若过滤、样本集合和模型同时改变，就不能把差异归因于批次处理本身。

| 当前任务 | 优先比较什么 | 哪种结果需要收紧结论 |
|---|---|---|
| 估计口腔部位关联 | 原始测量尺度上加入设计允许的板因素，与未调整结果并列 | 调整后方向反转，或区间明显扩大 |
| 检查某块板是否推动结果 | 保持同一目标比较，逐板留出并记录可用受试者及部位 | 去掉某板后主要比较不再有足够对照 |
| 预测新的受试者或实验批次 | 在训练数据内估计预处理，再对留出单位评估 | 随机分样本有效，而整板或整人留出后失效 |

这些检查有不同解释。效应改变提示结果依赖批次处理，不自动说明哪一个估计正确；可用对照消失则是设计支持不足。当前数据中 plate 的群落解释率约为 1%，也不能仅凭数值小就忽略：群落总体效应较小，仍可能对少数关键特征产生明显影响。

## 区分部位、受试者与实验板的影响 {#sec-code}
...
## 判断校正是否保留了生物差异 {#sec-polish}

### Batch × habitat 设计热图

```{r}
#| results: hold
#| label: reader-step-15
#| fig-show: hide
...
## 报告批次控制与敏感性分析的方法与限制 {#sec-methods}
...
## 将批次控制与敏感性分析用于自己的研究 {#sec-own-data}
