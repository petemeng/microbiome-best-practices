...
reader-reproduction:
  ...
  script: examples/31-da-methods.R
  packages:
  - ALDEx2
  - ANCOMBC
  - Maaslin2
...
### 方法不一致时，先查估计目标，再查显著名单

同一菌可以在 log-ratio、相对丰度均值和检出相关模型中表现不同，因为这些方法检验的量并不相同。先确认 contrast 方向、特征分母、零值处理、协变量和参与检验的样本一致，再解释差异；把所有系数都改名为 log fold change 并不会使它们成为同一效应量。

本例把数据与 CW 对 IW 固定，因而能观察建模假设本身的影响，但没有已知差异真值，不能用“发现得最多”或“被其他方法支持得最多”判断谁最准确。对不一致候选，更有帮助的是回看检出分布、极端值、收敛状态及可比尺度上的方向。确认性研究应预先选择与目标和设计相匹配的主要方法；其他方法用于暴露假设依赖，而不是提供取并集的额外发现机会。
...
[下载完整 R 脚本](https://github.com/petemeng/microbiome-best-practices/blob/691722146e287afe063bd43e9ce4fa41977fe763/examples/31-da-methods.R)。脚本包含数据下载、计算和图形导出。
...
## 比较效应、检验对象与方法一致性 {#sec-publication}
...
## 报告差异丰度方法与多重检验的方法与限制 {#sec-methods}
...
## 将差异丰度方法与多重检验用于自己的研究 {#sec-own-data}
