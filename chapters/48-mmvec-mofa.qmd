...
reader-reproduction:
  ...
  script: examples/48-mmvec-mofa.R
  packages:
  - ggplot2
  - jsonlite
  - patchwork
...
[Morton et al. (2019)](https://doi.org/10.1038/s41592-019-0616-3)用 MMvec 展示 microbe–metabolite conditional probabilities 与 embeddings；[Argelaguet et al. (2020)](https://doi.org/10.1186/s13059-020-02015-1)用 MOFA+ 展示多视图 factors、variance explained 和 loadings。
...
MOFA factor 的正负号可以整体翻转，排列也可能在重新拟合时改变；跨运行比较应匹配载荷和样本结构，而不是机械比较 Factor 1。某个 factor 与 diagnosis 相关，可以帮助描述数据，却不能仅据最强载荷给它命名为已确定的生物通路。还应看各 view 的解释比例、缺失模式与批次关联。

MMvec 的条件共现分数与 Spearman 相关估计对象不同，所以 top-50 零重叠不构成某一方错误的证据。本例 holdout MAE 最低点仍在第 100 轮，说明尚未观察到内部最优点；若继续用同一 holdout 反复选训练轮数和模型，它就成为验证集，需要另留测试数据才能无偏评估最终性能。两种模型都应在明确目标下与简单基线比较，不能用高维训练相关作为共同的成功指标。
...
[下载完整 R 脚本](https://github.com/petemeng/microbiome-best-practices/blob/691722146e287afe063bd43e9ce4fa41977fe763/examples/48-mmvec-mofa.R)。脚本包含数据下载、计算和图形导出。
...
conda env create --prefix ./multiomics-env --file env/multiomics.yml
```
...
完整计算脚本：[run_article48_native_mmvec.py](https://github.com/petemeng/microbiome-best-practices/blob/3cb6a817e0c73ab7ecbec1cedc1ad80cbb9ecfaa/scripts/run_article48_native_mmvec.py)、[run_article48_mmvec_mofa.py](https://github.com/petemeng/microbiome-best-practices/blob/3cb6a817e0c73ab7ecbec1cedc1ad80cbb9ecfaa/scripts/run_article48_mmvec_mofa.py)。
...
MMvec 的条件概率与 MOFA 的共享因子需要分别解释，不能把二者当成相同的关联分数。
...
## 报告MMvec与MOFA模型的方法与限制 {#sec-methods}
...
## 将MMvec与MOFA模型用于自己的研究 {#sec-own-data}

1. 替换三件套与 metabolite matrix/annotation；两个 views 必须是同一批 biological samples，而不是各自的 group averages。
2. 根据 detection/prevalence 和 measurement QC 不使用结局信息 地定 feature universe；不要从全模型结果倒推筛选阈值。
