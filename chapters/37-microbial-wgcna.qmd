...
reader-reproduction:
  ...
  script: examples/37-microbial-wgcna.R
  packages:
  - WGCNA
  - ggdendro
  - ggplot2
...
如果先按组间差异筛菌，再用同一批菌构建模块并检验模块与分组的相关，分组信息已经参与了两次选择。这里使用不依赖分组标签的过滤，避免这种直接循环；但共同地理梯度仍可能主导模块。模块与 pH 相关且与 Group 相关，不能被当作两个独立机制。[WGCNA 官方说明](https://edo98811.github.io/WGCNA_official_documentation/faq.html)也不建议把差异筛选后的特征当作无监督网络的默认输入。
...
[下载完整 R 脚本](https://github.com/petemeng/microbiome-best-practices/blob/691722146e287afe063bd43e9ce4fa41977fe763/examples/37-microbial-wgcna.R)。脚本包含数据下载、计算和图形导出。
...
### Module eigengene–trait correlation 与统一 BH
...
## 把模块关系与候选hub分开解释 {#sec-publication}
...
## 将模块识别与性状关联用于自己的研究 {#sec-own-data}
