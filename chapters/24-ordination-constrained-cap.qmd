---
title: "约束排序：CAP/db-RDA/RDA/CCA（复现）"
author: "Peter"
date: last-modified
format:
  html:
...
reader-reproduction:
  ...
  script: examples/24-ordination-constrained-cap.R
  packages:
  - ggplot2
  - ggrepel
  - scales
...
本例继续使用中国湿地土壤 16S 数据。[原始研究](https://doi.org/10.1016/j.geoderma.2018.09.035)报告 pH 与电导率是影响湿地土壤细菌群落的重要环境因子；[`microeco`](https://doi.org/10.1093/femsec/fiaa255)公开分发了 OTU、分类、样本和环境因子表。下面从这些表拟合一个五变量 CAP 模型。
...
CAP 图中的 pH 箭头表达当前约束空间内的梯度；边际检验问的是控制其余已纳入变量后，pH 是否仍提供额外解释。这两者都不自动等于改变 pH 所造成的群落变化。如果 pH 与未测量的地理或管理因素共同变化，统计调整只能控制已测且正确建模的部分。

本例五个环境变量的 VIF 均低于 2，说明这五列之间没有严重线性冗余，不代表混杂已经消失。加入 `Condition(Group)` 会把问题改为“同一地理组内的环境关联”；若变量主要在组间变化，估计会变得不精确，这不必然是模型失败，而是数据对新问题提供的信息更少。是否调整 Group 应由目标比较决定，不应由哪个模型更显著决定。

还有一个容易混淆的细节：在相同样本、带截距且无惩罚的线性约束模型中，给连续预测变量做 Z-score 不改变它们张成的空间，因此不会单靠缩放改变整体拟合或消除共线性。标准化便于描述单位；这与先计算环境欧氏距离时各变量量纲会改变权重的情况不同。
...
[下载完整 R 脚本](https://github.com/petemeng/microbiome-best-practices/blob/691722146e287afe063bd43e9ce4fa41977fe763/examples/24-ordination-constrained-cap.R)。脚本包含数据下载、计算和图形导出。
...
4. **把标准化当成消除共线性或混杂。** Z-score 便于比较变量单位，但不会改变本例线性约束模型的解释空间，也不能产生缺失的组内对照。对数变换、变量选择与调整集则可能实质改变问题，应分别说明。
...
## 将约束排序与置换检验用于自己的研究 {#sec-own-data}
