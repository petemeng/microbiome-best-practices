...
reader-reproduction:
  ...
  script: examples/30-compositional-data.R
  packages:
  - ggplot2
  - ggrepel
  - knitr
...
[Gloor et al. (2017)](https://doi.org/10.3389/fmicb.2017.02224)系统解释了为什么高通量测序表是 composition，而不是绝对丰度表。下面用公开湿地 16S 计数展示三类直接影响分析的证据：真实组成上的反事实闭合实验、同一属在三种测量尺度下的组间分布，以及 reference 改变时效应量如何移动。
...
本页数据来自 `microeco` 2.0.0 随包分发的真实湿地 16S 数据，包含 coastal wetland（CW，滨海湿地）、inland wetland（IW，内陆湿地）和 Tibetan wetland（TW，青藏高原湿地）各 30 个样本。属级已知 reads 仅占全部 reads 的一部分，未注释部分保留为 denominator residual，不被解释成一个生物学 taxon。

### 换一个 reference 后，为什么“增加”可能变成“减少”？

ALR 的组间效应是目标菌与参照菌的变化之差。换参照后符号改变，可能只是参照本身变化，并不说明前一次计算出错。CLR 把参照换成入选特征的几何均值，因此过滤或改变分类层级也可能移动效应基线。报告 log-ratio 结果时，应同时说明参照组成和目标对比，而不是只写“丰度上调”。

一个两菌比例在删除其他正值组分后仍保持不变，但对计数加同一个伪计数再取比例，不再具有完全相同的尺度性质。低深度、低检出菌尤其可能受影响。若结论依赖某个伪计数或少数参照菌，应报告敏感性；若科学问题确实是每克样本增加多少，需要独立的微生物负荷测量，不能从另一种归一化中恢复这个缺失尺度。
...
[下载完整 R 脚本](https://github.com/petemeng/microbiome-best-practices/blob/691722146e287afe063bd43e9ce4fa41977fe763/examples/30-compositional-data.R)。脚本包含数据下载、计算和图形导出。
...
## 将组成数据变换与参照用于自己的研究 {#sec-own-data}
