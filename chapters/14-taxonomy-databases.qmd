---
title: "第 14 篇 · 物种注释 + 数据库选择（SILVA/GTDB/Greengenes2）"
...
> **分析目标：**给同一批真实 16S V4 ASV 分别匹配 SILVA 138.2、GTDB R11-RS232 与 Greengenes2 2024.09，分开回答“数据库覆盖了什么”“短扩增子能注释到哪一层”“不同名称是否真的代表生物差异”。  
> **输入文件：**366 条真实 V4 ASV、对应的 5,213 条非嵌合 reads 频数，以及从三个官方完整发布用同一 515F/806R 合同提取并做 exact-sequence LCA 去冗余的参考序列与 taxonomy。  
> **预计资源：**区域参考资产压缩后约 22 MB；4 线程导入、maximum validation、三次 consensus-VSEARCH 分类和审计通常约 15–25 分钟，建议预留 8 GB 内存。若从三个完整官方发布重新提取区域参考，需要额外磁盘、网络和更长的一次性运行时间。  
> **执行策略：**完整发布下载、区域提取、LCA 去冗余、三库分类、Artifact validation 与 provenance 审计均已一次性实跑；网页构建使用 `eval: false`，避免重复执行上游重任务。
...
### 6. 同时报告 ASV 比例和 read-weighted 比例
...
### 7. species label 不等于 species-level resolution
...
### 10. 预训练 sklearn classifier 还有版本合同
...
<details>
<summary><strong>展开：固定环境、真实数据、参考资产与作图函数</strong></summary>
...
### 4. 导出 taxonomy 与 search results
...
### 1. 主图优先展示逐级覆盖，而不是只给一个 assignment %
...
### 2. reference scope 用对数轴，但不要暗示“大就是准”
...
### 3. concordance 热图必须写“exact-name”
