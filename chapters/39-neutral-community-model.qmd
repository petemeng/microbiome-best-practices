...
> **分析目标：**用 Sloan neutral community model（NCM）拟合 OTU 的 metacommunity mean relative abundance 与 sample occupancy，估计 immigration parameter $m$，识别高于、低于和落入 neutral predictive envelope 的 OTU，并审计 detection rule、测序深度和 regional pool 对结论的影响。  
> **输入文件：**`data/small/otutab.tsv`、`taxonomy.tsv`、`metadata.tsv`。  
> **预计资源：**13,628 个 OTU、90 个样本；主模型、分组模型、800 次 sample bootstrap、一次 10,000-read rarefaction 与 17 个 detection thresholds，通常 2–5 分钟、峰值内存低于 2 GB；四张图导出为 PDF/SVG/600 ppi PNG/LZW-TIFF。

## 中性模型的偏离不等于确定性机制 {#sec-target}

[Sloan et al. (2006)](https://doi.org/10.1111/j.1462-2920.2005.00956.x)给出了微生物 neutral community model；[Burns et al. (2016)](https://doi.org/10.1038/ismej.2015.142)用 abundance–occupancy 曲线比较斑马鱼不同发育阶段的中性拟合。这里不转载论文原图，而是在公开湿地 16S 数据上重绘 abundance–occupancy envelope、分组 immigration estimate、classification sensitivity 和 detection-limit sensitivity。
...
## 理论：曲线、参数和偏离各自回答什么 {#sec-theory}

### 1. abundance–occupancy 是模型的观测接口
...
### 2. envelope 是 predictive classification，不是 taxon 性格测试
...
### 4. regional pool 和 detection rule 是 estimand 的组成部分
...
### 2. 拟合 Sloan NCM 并构造 predictive envelope
...
### 3. 对 $m$ 做 sample bootstrap，并分层拟合三个 wetland pools
...
### 图 1：abundance–occupancy curve 与 95% envelope
...
### 图 4：detection limit sensitivity curve
