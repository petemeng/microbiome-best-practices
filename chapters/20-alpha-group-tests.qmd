...
reader-reproduction:
  ...
  script: examples/20-alpha-group-tests.R
  packages:
  - digest
  - dplyr
  - emmeans
...
### 不显著之后，怎样判断是差异小还是估计不精确？

本例最大的成对 Cliff's delta 为 0.258，95% bootstrap 区间为 −0.033 到 0.547。区间同时容许接近无差异和有一定幅度的秩差异，因此不能把结果写成“三种湿地的多样性相同”。若科学问题是证明差异小到可忽略，应在分析前给出有生物学意义的等效界值，并使用对应的等效性分析；普通检验的 P > 0.05 不能代替它。

bootstrap 也不自动解决伪重复。本例在每组内重抽样本，区间有效性依赖这些样本可作为独立单位。若多个采样点来自同一湿地，应按湿地保留整组观测进行重采样，或采用能表达层级的模型。90 个测序样本不一定提供 90 个独立生态重复；这项设计限制应与效应区间一起报告。
...
[下载完整 R 脚本](https://github.com/petemeng/microbiome-best-practices/blob/691722146e287afe063bd43e9ce4fa41977fe763/examples/20-alpha-group-tests.R)。脚本包含数据下载、计算和图形导出。
...
## 同时展示样本分布、效应与区间 {#sec-publication}
...
## 报告Alpha多样性组间比较的方法与限制 {#sec-methods}
...
## 将Alpha多样性组间比较用于自己的研究 {#sec-own-data}
