...
> **分析目标：**在重复测量设计中，用线性混合模型估计 Shannon 多样性的组别×时间效应，用 Bray–Curtis 轨迹和相邻时间点 volatility 描述个体内群落变化。  
> **输入文件：**`otutab.tsv`、`taxonomy.tsv`、`metadata.tsv`；metadata 还需包含个体 ID、组别和有序时间点。  
> **预计资源：**真实示例为 130 个 HITChip 分类单元、222 个样本、38 位参与者和 6 个时间点；普通笔记本约 2–5 分钟，内存低于 1 GB。
...
HITChip 不是扩增子测序；它的丰度预处理不能照搬到 ASV 表，但“特征×样本 → 样本内指标 → 重复测量模型”的下游逻辑相同。这里不转载论文原图，而是从公开表格重算四张图。

::: {.callout-important title="纵向连线不等于纵向检验"}
把同一个体的点连起来只是一种展示。推断必须在模型中声明同一个体内的相关性；把 222 个样本当作 222 个独立观察，会低估标准误并制造过度显著。
:::

## 理论：为什么普通线性模型不够 {#sec-theory}
...
下面把固定版本源码包中的 `dietswap` 对象导出为标准三件套。SHA-256 可防止下载内容悄然变化。

```{r}
#| eval: false
archive <- "data/raw/microbiome_1.26.0.tar.gz"
url <- paste0(
  "https://bioconductor.org/packages/3.19/bioc/src/contrib/",
  "microbiome_1.26.0.tar.gz"
)
expected <- paste0(
  "67cc6d117ce9dfbbc472fba828e1bb0d",
  "48c621fb574f4f8d3424f6cc9a77fdab"
)
dir.create("data/raw", recursive = TRUE, showWarnings = FALSE)
dir.create("data/small/longitudinal-dietswap", recursive = TRUE, showWarnings = FALSE)
if (!file.exists(archive)) download.file(url, archive, mode = "wb")
stopifnot(identical(
  digest::digest(archive, algo = "sha256", file = TRUE), expected
))

tmp <- tempfile("dietswap-")
dir.create(tmp)
utils::untar(archive, files = "microbiome/data/dietswap.rda", exdir = tmp)
e <- new.env(parent = emptyenv())
load(file.path(tmp, "microbiome", "data", "dietswap.rda"), envir = e)
ps <- e$dietswap

original_names <- phyloseq::taxa_names(ps)
feature_ids <- sprintf("HT_%03d", seq_along(original_names))
otutab <- as(phyloseq::otu_table(ps), "matrix")
rownames(otutab) <- feature_ids
tax0 <- as.data.frame(as(phyloseq::tax_table(ps), "matrix"))
taxonomy <- data.frame(
  Kingdom = "Bacteria", Phylum = tax0$Phylum, Class = "", Order = "",
  Family = tax0$Family, Genus = tax0$Genus, Species = "",
  DisplayName = original_names, row.names = feature_ids,
  check.names = FALSE
)
meta0 <- as.data.frame(phyloseq::sample_data(ps))
metadata <- data.frame(
  SubjectID = meta0$subject,
  Sex = tools::toTitleCase(as.character(meta0$sex)),
  Cohort = ifelse(meta0$nationality == "AAM", "African American", "Rural African"),
  OriginalCohortCode = meta0$nationality,
  PhaseCode = meta0$group,
  Timepoint = as.integer(meta0$timepoint),
  TimeWithinPhase = as.integer(meta0$timepoint.within.group),
  BMIGroup = tools::toTitleCase(as.character(meta0$bmi_group)),
  row.names = meta0$sample,
  check.names = FALSE
)
...
write_piece(otutab, "FeatureID", file.path(base, "otutab.tsv"))
write_piece(taxonomy, "FeatureID", file.path(base, "taxonomy.tsv"))
write_piece(metadata, "SampleID", file.path(base, "metadata.tsv"))
...
整套项目运行时可改用内容相同的 `R/theme_pub.R`；复制单篇时保留上面的定义即可。
