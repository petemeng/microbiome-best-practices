...
一个人的指标随时间升高，与平均指标较高的人属于哪个群体，是不同的比较。随机截距吸收稳定个体差异，但不会自动分离所有时间变化的混杂。若饮食、药物或疾病活动也随访变化，应先确定目标效应，再决定是否需要分解协变量的个体均值与个体内偏离。

相邻观测距离还取决于时间间隔。两天与两周的 Bray–Curtis 差异不能直接当作同一速度比较；缺失中间时间点会让“相邻记录”跨过更长间隔。本例只有 38 位参与者，随机斜率和复杂残差结构需要与信息量相匹配。AR(1) 若按访问序号而非真实时间建立，也隐含等间隔假设，应与实际采样记录核对。
