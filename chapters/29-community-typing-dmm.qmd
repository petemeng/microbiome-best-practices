...
八个初始化得到同一 partition，解决的是“优化器有没有停在不同局部解”的问题；每人只留一份样本后最优 k 从 4 变为 2，涉及的则是“当前采样权重是否支持四个成分”。重复样本较多的参与者会对拟合贡献更多信息，不能把初始化稳定当成已解决这种设计依赖。

即使最大后验概率接近 1，它也只是给定当前候选模型下的归属，不证明自然界存在离散边界。DMM 可以用多个成分近似连续梯度。若目标是给新样本分型，应冻结特征集合与拟合参数，再评价独立样本的预测概率和低置信归属；不能在新队列重新聚类后，仅凭颜色相似就认定复现了同一种状态。若要研究分型与 BMI 的关系，还须处理重复个体、混杂和归属不确定性。

模型选择、后验着色 PCoA、component profile 热图和稳定性比较应一起解读。
