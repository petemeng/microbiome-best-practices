...
> **分析目标：**用 Dirichlet-multinomial mixture（DMM，Dirichlet–多项式混合模型）从原始属级计数估计概率性群落状态，同时审计成分数、初始化、标签交换、后验不确定性、特征过滤、测序深度与重复样本稳定性。  
> **输入文件：**`otutab.tsv`、`taxonomy.tsv`、`metadata.tsv`。  
> **预计资源：**示例含 278 个样本、130 个来源属级类别和 570,851 reads；42 个 fresh fits 各用一次性 R 进程、最多 8 个并发，通常需 3–6 分钟，峰值内存约 1–2 GB，四张图以 PDF/SVG/600 ppi PNG/LZW-TIFF 导出。

## 群落分型是概率模型，不是天然类别 {#sec-target}

群落分型常见于论文中的三类图：模型证据随 component 数 `k` 的曲线、按“肠型”（enterotype）或阴道群落状态型（community state type, CST）着色的排序图，以及各状态优势类群的组成热图。Holmes 等人的原文可直接查看 [Figure 1：Laplace model evidence](https://doi.org/10.1371/journal.pone.0030126.g001)、[Figure 2：按四个 component 着色的 NMDS](https://doi.org/10.1371/journal.pone.0030126.g002) 和 [Table 4：BMI 与 component 的列联表](https://doi.org/10.1371/journal.pone.0030126.t004)。PLOS ONE 原文采用 CC BY 许可；这里不嵌入原图，而是沿用“模型选择—排序显示—component 解释”的信息结构，从公开计数重绘并增加不确定性与稳定性审计。真正需要回答的不是“软件能分成几类”，而是：**离散状态是否比连续梯度更有支持，样本属于某状态的把握有多大，换一种合理的数据处理后结论是否仍成立。**
...
这里不复制论文原图，而是从公开计数重新绘制四张原创图：模型选择、后验着色 PCoA、component profile 热图和稳定性账本。

## 理论：DMM 给的是概率状态，不是血型式类别 {#sec-theory}

### 1. 单个 Dirichlet–多项式同时描述组成和过度离散
...
### 5. Enterotype 与 CST 需要不同语境的外部证据
...
<details>
<summary><strong>展开：安装依赖、导出真实三件套和定义作图函数</strong></summary>

R 4.4 与 Bioconductor 3.19 的锁定安装：
...
从 `DirichletMultinomial` 1.46.0 的 `extdata` 导出标准三件套。`otutab` 保持 feature × sample；taxonomy 只在 `Genus` 填入来源标签，因为包内没有分发其余 rank：
...
stopifnot(
  digest::digest(twins_path, algo = "sha256", file = TRUE) ==
    "b6a231acded317b49b7fd800bf8b4ce12a75340c5b88499b51e92d72528f93ba",
  digest::digest(phenotype_path, algo = "sha256", file = TRUE) ==
    "4286c3f9255f7f1ce7f5c5ac5e60d4e27284fe358ceb553108748c67ab4d8b05"
)

otutab <- as.matrix(read.csv(
  twins_path, row.names = 1, check.names = FALSE
))
storage.mode(otutab) <- "integer"
...
taxonomy <- data.frame(
  ...
  Family = NA_character_, Genus = rownames(otutab),
...
metadata <- data.frame(
  BMIClass = c("Lean", "Obese", "Overweight")[bmi_code + 1L],
  BMIClassCode = bmi_code,
  ParticipantKey = participant_key,
  SourceSuffix = ifelse(
    grepl("\\.2$", sample_ids), "Dot2 suffix", "No suffix"
  ),
  SamplesPerParticipantKey = as.integer(
    table(participant_key)[participant_key]
  ),
...
dir.create(
  "data/small/community-typing-dmm",
  recursive = TRUE,
  showWarnings = FALSE
)
write_keyed_tsv(
...
出版级函数直接内联。图内统一使用英文、DejaVu Sans 和色觉友好配色：
...
这里不复用 worker：每批最多并发 8 个 PSOCK worker，而每个 worker 只完成一个 fit 就退出。每个进程都显式限制为单线程并接收独立 seed。在本次环境中，复用 worker 时，同一个 `dmn(seed=...)` 曾因前序 fit 历史而进入不同解；一次性进程把进程状态也纳入确定性合同。并行只缩短墙钟时间，不改变每个 fit 的输入、seed 或选择规则。

### 5. 同时查看 Laplace、AIC 和 BIC
...
### 6. Canonicalize component，并保留全部 posterior
...
### 7. 区分 fresh fit 与包内 reference fit
...
### 图 2：posterior uncertainty 进入排序图
...
### 图 3：展示完整 component profile，而不是贴单菌标签
