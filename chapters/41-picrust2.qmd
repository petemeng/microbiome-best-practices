---
title: "PICRUSt2 + ggpicrust2：从 ASV 到通路热图"
author: "Peter"
date: last-modified
format:
  html:
...
本例只有 37 条 ASV，许多通路还共享相同的预测来源。多个显著通路未必是多条独立机制；应查看其共同的类群贡献以及 Facility 调整后的结果。若关键结论依赖特定功能，最有价值的补充是相同样本或独立样本中的直接功能测量，而不是再换一个热图聚类方式。[PICRUSt2 的参考库更新研究](https://pmc.ncbi.nlm.nih.gov/articles/PMC12089645/)也表明，参考范围是预测质量的重要条件。
...
## PICRUSt2 如何根据系统发育邻近关系推断基因家族 {#sec-theory}
...
## 结合NSTI解释预测功能 {#sec-beautify}

完成上面的 PICRUSt2 计算后，用下面的 R 命令读取三张表与本次预测输出，重新计算按 facility 调整的通路比较，并导出四张结果图：

```bash
Rscript --vanilla scripts/render_article41_picrust2.R .
```

[查看完整的通路分析与绘图脚本](https://github.com/petemeng/microbiome-best-practices/blob/3cb6a817e0c73ab7ecbec1cedc1ad80cbb9ecfaa/scripts/render_article41_picrust2.R)。NSTI、通路丰度和 coverage 均来自前面的预测输出，不是直接测量的基因表达或代谢通量。
...
## 将PICRUSt2预测与局限用于自己的研究 {#sec-own-data}
